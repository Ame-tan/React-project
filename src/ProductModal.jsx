...
import { auth } from "./firebase"; // 確保正確引入 firebase
...
const ProductModal = ({ product, images, onClose }) => {
  ...
  const handleAddToCartAlert = () => {
    ...
    if (product.category === "飾品") {
      ...
      } else if (product.id === "12" && !color.length) {
        // 檢查id為12 且 未選擇顏色
          ...
          <div className="relative mx-auto   lg:pl-8 h-full ">
            ...
            <button
              ...
              disabled={product.category === "飾品"
                ...
                : !(size && goods.length > 0 && (product.id !== "12" || color.length > 0))}
              className={`lg:mt-10 mt-4 py-2 rounded-md w-full ${
                size && goods.length > 0 && (product.id !== "12" || color.length > 0)||(product.category === "飾品" && goods.length>0)
                  ? "bg-red-400 text-white"
